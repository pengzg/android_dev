 ...
 */
public class  UnifyPurchaseFragment extends BaseFragment implements ClassifyTwoAdapter.ClassifyTwoListener,
...
//    public UnifyPurchaseFragment(PurchaseFragment purchaseFragment) {
...
//    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_unify, container, false);
    ...
    public void OnActCreate(Bundle savedInstanceState) {
        ...
        selectPwSynthesize();
        selectPwScreen();
        selectEditCartPwScreen();//初始化editCartPopup
        loadCartPriceNum();
    ...
    private void loadClassifyData(int isFirst) {
        ...
        httpUtil.post(M_Url.GoodsCategory, L_RequestParams.
                getGoodsCategory(UserInfoUtils.getId(getActivity()),"1",UserInfoUtils.getStoreHouseId(getActivity())), false);
    }
    ...
    private void setTwoClassifyList(int oneIndex) {
        ...
        if (classBean.getListData().get(oneIndex).getSubList() != null &&
            ...
            categoryId = twoClassifyList.get(0).getGcId();
            itemClassifyTwo(0);
        } else {
            //没有二级分类信息
           /* categoryType = classBean.getListData().get(oneIndex).getBackType();
            this.categoryCode = classBean.getListData().get(oneIndex).getGcCode();
            //this.categoryCode = "";
            categoryId = classBean.getListData().get(oneIndex).getGcId();
            loadGoodsList();*/
    ...
    public void itemClassifyTwo(int position) {
        ...
        this.categoryCode = twoClassifyList.get(position).getGcCode();
        categoryId = twoClassifyList.get(position).getGcId();
        loadGoodsList();
    }
    ...
    private void loadGoodsList() {
        ...
        httpUtil.post(M_Url.GoodsList, L_RequestParams.getGoodsList(UserInfoUtils.getId(getActivity()),categoryType,
                categoryCode, String.valueOf(pageNo), "1",String.valueOf(conditionType), strBrandId, strPriceId,
                strCategoryId,UserInfoUtils.getStoreHouseId(getActivity())), true);
    }
    ...
    public void itemGoods(int position) {
        ...
        intent.putExtra("gpId", mGoodsBeanList.get(position).getGgp_id());//价格id
        intent.putExtra("gpsId",mGoodsBeanList.get(position).getGps_id());//价格方案id
        startActivity(intent);
    }
    ...
    public void onEventMainThread(ConfirmOrderEvent event) {
        boolean isRefresh = false;
        for (int i=0;i<event.getDataList().size();i++){
            for (int j=0;j<mGoodsBeanList.size();j++){
                if (event.getDataList().get(i).equals(mGoodsBeanList.get(j).getGg_id())){
                    mGoodsBeanList.get(j).setCartnum("0");
                    isRefresh = true;
                }
            }
        }
