...
public class RolloutGoodsOrderDetailAdapter extends BaseAdapter {
    ...
    public RolloutGoodsOrderDetailAdapter(List<PHOrderDetailBean> list) {
        this.list = list;
        notifyDataSetChanged();
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if ("".equals(bean.getPhSaleNum_desc()) || bean.getPhSaleNum_desc() == null) {
            holder.mTvSalenumDesc.setText("已结算：" + "0");//已销售数量描述
        } else {
            holder.mTvSalenumDesc.setText("已结算：" + bean.getPhSaleNum_desc());//已销售数量描述
        }
        if ("".equals(bean.getResidueNum_desc()) || bean.getResidueNum_desc() == null) {
            holder.mTvSynumDesc.setText("未结算：" + "0");//剩余数量描述
        } else {
            holder.mTvSynumDesc.setText("未结算：" + bean.getResidueNum_desc());//剩余数量描述
        }
        return view;
    }
