...
public class CartActivity extends BaseActivity implements CartListener {
    ...
    public void cartAction(int i) {//商品的活动跳转
        if (list.get(i).getActivityType().equals("5")) {
            intent = new Intent(this, ActionActivityGroup.class);
    ...
    private  void modifyCart(final int num, final int i){
        ...
        AsyncHttpUtil<GoodsCartBean> httpUtil = new AsyncHttpUtil<>(CartActivity.this, GoodsCartBean.class, new IUpdateUI<GoodsCartBean>() {
            @Override
        ...
        httpUtil.post(M_Url.editCartGoodNum,L_RequestParams.editCart(UserInfoUtils.getId(this),"", String.valueOf(num),
                UserInfoUtils.getStoreHouseId(this),list.get(i).getActivityId()),false);
    }
    ...
    private void loadCartList(int isFirst, boolean isDialog) {
        ...
        httpUtil.post(M_Url.cartList, L_RequestParams.getCartList(UserInfoUtils.getId(this), UserInfoUtils.getStoreHouseId(this)), isDialog);
    }
    ...
    @Override
    public void cartChildDelete(final int i, int j) {
        ...
        /*if ("5".equals(list.get(i).getActivityType())) {
            ...
            DialogUtil.showCustomDialog(this, "提示", "是否删除这个套餐?", "确定", "取消", new DialogUtil.MyCustomDialogListener2() {
                @Override
                public void ok() {
                    deleteCart(str, i);
                }
    ...
    public void cartPlus(int i, int j) {
        ...
        /* for (int a=0;a<list.size();a++){
            if (a == i)continue;//如果在相同的index,则跳过
            for (int b=0;b<list.get(a).getDataList().size();b++){
                CartGoodsListBean goodsBean = list.get(a).getDataList().get(b);
                if (bean.getC_id().equals(goodsBean.getC_id())){

                    BigDecimal cartNum;//商品在购物车中数量
                    if (!TextUtils.isEmpty(bean.getC_goods_num()) && !"".equals(bean.getC_goods_num())){
                        cartNum = new BigDecimal(bean.getC_goods_num());
                    }else{
                        cartNum = BigDecimal.ZERO;
                    }

                    plusNum = plusNum + cartNum.intValue();
                }
            }
        }*/
    }
    ...
    public void cartEdit(final int i, final int j) {
        ...
        DialogUtil.showEditCartNum(this, "加入购物车数量", "确定", "取消", bean.getC_goods_num(),
     ...
     * 删除购物车商品
     *
     * @param comboIndex 套餐id,-1不是,其余是
     */
    private void deleteCart(final String cartIds, final int comboIndex) {
        AsyncHttpUtil<CartBean> httpUtil = new AsyncHttpUtil<>(this, CartBean.class, new IUpdateUI<CartBean>() {
            @Override
            public void updata(CartBean jsonBean) {
                if (jsonBean.getRepCode().equals("00")) {
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        for (int k = 0; k < list.get(i).getDataList().size(); k++) {
                            for (int j = 0; j < strCartIds.length; j++) {
                                if (list.get(i).getDataList().get(k).getC_id().equals(strCartIds[j])) {
        ...
        httpUtil.post(M_Url.deleteCart, L_RequestParams.getDeleteCart(
                UserInfoUtils.getId(this), cartIds, UserInfoUtils.getCenterShopId(this), UserInfoUtils.getStoreHouseId(this)), true);
    }
    ...
    public void clearCart() {
        ...
        httpCleart.post(M_Url.clearCart, L_RequestParams.
                getCleartCart(UserInfoUtils.getId(this), UserInfoUtils.getCenterShopId(this), UserInfoUtils.getStoreHouseId(this)), true);
    }
    ...
    private void alterCart(final boolean isDelete, final int i, final int j,
        ...
        httpUtil.post(M_Url.editCartGoodNum, L_RequestParams.editCart(UserInfoUtils.getId(this), ggpId, goodsNum, UserInfoUtils.getStoreHouseId(this),
                ""), false);
    }
