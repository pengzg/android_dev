...
import com.xdjd.distribution.activity.SetGestureActivity;
...
public class GestureLockView extends View {
    ...
    private float baseYPos = 100;//Y坐标起始位置
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (currentStatus) {
            case STATUS_DEFAULT:
                initPaint.setColor(Color.parseColor("#212121"));//黑色
                for (int i = 0; i < 9; i++) {
                    canvas.drawCircle(baseXPos + i / 3 * offset, baseYPos + i % 3 * offset, 60, initPaint);
                }
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    if (!list.contains(i)) {
                        if (x >= pointX - 80 && x <= pointX + 80 && y >= pointY - 80 && y <= pointY + 80) {
                            ...
                            list.add(i);
                            vibrator.vibrate(50);
                            initPaint.setColor(Color.parseColor("#51C5C5"));
                            initPaint.setStyle(Paint.Style.FILL);
                            canvas.drawCircle(pointX, pointY, 30, initPaint);
                        } else {
                            initPaint.setColor(Color.parseColor("#212121"));
                        }
                    } else {
                        initPaint.setColor(Color.parseColor("#51C5C5"));
                        initPaint.setStyle(Paint.Style.FILL);
                    ...
                    initPaint.setStyle(Paint.Style.STROKE);
                    canvas.drawCircle(pointX, pointY, 60, initPaint);
                }
                if (list.size() > 0) {
                    choosenPaint.setColor(Color.parseColor("#51C5C5"));//连接线颜色
                    Path path = new Path();
