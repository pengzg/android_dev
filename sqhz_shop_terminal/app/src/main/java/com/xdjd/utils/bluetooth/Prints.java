...
public class Prints {
    ...
    public static <T> boolean printReceiptYsk(BluetoothSocket bluetoothSocket, ClientBean clientBean, String salesmanName,
        ...
        try {
            ...
            pUtil.printLargeText("收欠款-收欠款", 1);
            pUtil.printLine();
     ...
     *
     * @param bluetoothSocket
     * @param type            打印订单类型 1.申报;2.车销;3.配送任务;4.出库查询;5要货申请;6退货申请;7.订单查询;8订货;9.去铺货;10.铺货销售;11.铺货撤货;
     *                        12.铺货单;13.销售单;14.撤货单;15.返陈列;16返陈列查询中打印
     * @param title           订单类型
     * @param time            时间
     ...
     * @return
     */
    public static <T> boolean printOrder(BluetoothSocket bluetoothSocket, int type, ClientBean clientBean, String title, String time, String orderCode,
                                         String salesmanName, String salesmanPhone, T t, String orderAmount, boolean isPrintCode, String note) {
        return printOrder(bluetoothSocket, type, clientBean, title, time, orderCode, salesmanName, salesmanPhone, t, orderAmount,
                null, null, null, null, null, null, isPrintCode, null, note);
    }
    ...
    public static <T> boolean printOrder(BluetoothSocket bluetoothSocket, int type, ClientBean clientBean, String title, String time, String orderCode,
                                         ...
                                         boolean isPrintCode, T t1, String note) {
        //是否打印条码,true:打印
        boolean isPrintGoodsCode = false;

        try {
            PrintUtil pUtil = new PrintUtil(bluetoothSocket.getOutputStream(), "GBK");
            headPrint(pUtil, title);//头部信息
            if (type != 5 && type != 6 && clientBean != null) {
                clientPrint(pUtil, clientBean);//客户信息
            }

            if (orderCode != null && !"".equals(orderCode)) {//单号不为空时才打印
            ...
            switch (type) {
                case 1:
                case 2:
                    for (OrderSettlementBean bean : (List<OrderSettlementBean>) t) {
                ...
                case 6:
                case 15:
                    for (GoodsBean bean : (List<GoodsBean>) t) {
...
//                            goodsNum = bean.getMaxNum() + bean.getGg_unit_max_nameref() + bean.getMinNum() + bean.getGg_unit_min_nameref();
                        ...
                        goodsNum = UnitCalculateUtils.goodsNumDesc(bean.getGgp_unit_num(), bean.getMaxNum(), bean.getMinNum(),
                                bean.getGg_unit_max_nameref(), bean.getGg_unit_min_nameref());

                        printGoodsData(pUtil, bean.getGg_title(), "", bean.getTotalPrice(), goodsNum, goodsPrice,
                                bean.getGg_international_code_min(), isPrintCode);
                    }
                    break;
                case 3:
                   /* for (DistributionGoodsBean bean : (List<DistributionGoodsBean>) t) {
                        //价格如果为0不打印
                        *//*BigDecimal totalAmount = BigDecimal.ZERO;
                        if (TextUtils.isEmpty(bean.getOd_real_total())) {
                            totalAmount = BigDecimal.ZERO;
                        } else {
                            totalAmount = new BigDecimal(bean.getOd_real_total());
                        }
                        if (totalAmount.compareTo(BigDecimal.ZERO) == 0) {//如果总价格等于0,则不打印改商品信息
                            continue;
                        }*//*
                        if ("1".equals(bean.getOd_unit_num())) {//大小单位换算比==1,不显示大单位
                            goodsNum = bean.getOd_goods_num_min() + bean.getOd_goods_unitname_min();
                        ...
                        pUtil.printFourData(bean.getOd_goods_name() + bean.getGg_model()+" / "+bean.getGg_international_code_min(), goodsNum,
                                goodsPrice, bean.getOd_real_total());
                    }*/
                    ...
                    break;
                case 4:
                    for (OutboundDetailBean bean : (List<OutboundDetailBean>) t) {
                    ...
                    break;
                case 7:
                    for (OrderDetailBean bean : (List<OrderDetailBean>) t) {
                        if ("1".equals(bean.getOd_unit_num())) {//大小单位换算比==1,不显示大单位
                            goodsNum = bean.getOd_goods_num_min() + bean.getOd_goods_unitname_min();
                            goodsPrice = bean.getOd_price_min();
                        } else {
                ...
                case 9://铺货
                    for (GoodsBean bean : (List<GoodsBean>) t) {
                        if ("1".equals(bean.getGgp_unit_num())) {//大小单位换算比==1,不显示大单位
                            //                            goodsNum = bean.getMinNum() + bean.getGg_unit_min_nameref();
                            goodsPrice = bean.getMinPrice().equals("")?"0.00":bean.getMinPrice();
                        } else {
                            //                            goodsNum = bean.getMaxNum() + bean.getGg_unit_max_nameref() +
                            //                                    bean.getMinNum() + bean.getGg_unit_min_nameref();
                            goodsPrice = (bean.getMaxPrice().equals("")?"0.00":bean.getMaxPrice()) + "/" +
                                    (bean.getMinPrice().equals("")?"0.00":bean.getMinPrice());
                        }
                ...
                case 10://铺货销售
                case 11://铺货撤货
                    for (PHOrderDetailBean bean0 : (List<PHOrderDetailBean>) t) {
                        ...
                        for (PHOrderDetailBean bean : bean0.getListGoodsData()) {
                            ...
                            printGoodsData(pUtil, goodsName, "", bean.getTotalPrice(), goodsNum, goodsPrice,
                                    bean.getGoodsCode(), isPrintCode);
                        }
                    }
                    break;
                case 12://铺货单
                    for (PHOrderDetailBean bean : (List<PHOrderDetailBean>) t) {
                        String priceStr;
                ...
                case 14:
                    ...
                    break;
                case 16:
                    for (DisplayInDetailBean bean : (List<DisplayInDetailBean>) t) {
                        if ("1".equals(bean.getEii_unit_num())) {//大小单位换算比==1,不显示大单位
                            goodsPrice = bean.getEii_goods_price_min();
                        } else {
                            goodsPrice = bean.getEii_goods_price_max() + "/" + bean.getEii_goods_price_min();
                        }
                        goodsNum = UnitCalculateUtils.goodsNumDesc(bean.getEii_unit_num(), bean.getEii_goods_quantity_max(), bean.getEii_goods_quantity_min(),
                                bean.getEii_unit_max(), bean.getEii_unit_min());

                        printGoodsData(pUtil, bean.getEii_goods_name(), "", bean.getEii_goods_amount(), goodsNum, goodsPrice,
                                "", isPrintCode);
                    }
                    break;
            }

            if (type == 2 || type == 3 || type == 10 || type == 13 || (type == 4 && clientBean != null && skAmount != null)) {
                pUtil.printDashLine();
                pUtil.printLine();
            ...
            pUtil.printDashLine();
            pUtil.printLine();
            if (type == 9 || type == 12) {//铺货下单是显示总欠款
                pUtil.printLargeText("欠款金额:" + orderAmount, 0);
            } else {
                pUtil.printLargeText("总金额:" + orderAmount, 0);
            }
            ...
            PHOrderDetailBean beanRollout = (PHOrderDetailBean) t1;
            if ((type == 14 || type == 13 || type == 11 || type == 10) &&
                    beanRollout != null && beanRollout.getListData() != null && beanRollout.getListData().size() > 0) {
                pUtil.printDashText("历史铺货单据");
               /* pUtil.printTextBold();
                if (UserInfoUtils.getPtinterType(UIUtils.getContext()) == BaseConfig.printer80) {
                    String leftOne = isPrintGoodsCode ? "商品" : "商品/商品码";
                    pUtil.printFourData(leftOne, "总数量", "单价", "总金额");
                    pUtil.printThreeData("已撤货", "已销售", "未销售");
                } else {//58mm打印格式
                    pUtil.printThreeData("数量", "单价", "总金额");
                    pUtil.printThreeData("已撤货", "已销售", "未销售");
                }
                pUtil.printTextBoldCancel();*/
            ...
            //                    Bitmap bitmap1 = null;
            //                    try {
            //                        bitmap1 = ZXingUtil.CreateBrandCode(orderCode, 720, 50);
            ...
            pUtil.printTwoColumnWeight("业务员:" + salesmanName, "电话:" + salesmanPhone);
            pUtil.printTwoColumnWeight("日期:", "客户签字:");
            if ((type == 9 || type == 11 || type == 12 || type == 14) && note != null && note.length() > 0) {
                pUtil.printText("备注:" + note);
                pUtil.printLine();
            }
            pUtil.printLine(1);
    ...
    public static <T> boolean printOrderList(BluetoothSocket bluetoothSocket, int type, ClientBean clientBean, String title, String time, String orderCode,
        ...
        try {
            ...
            if (type == 2) {//车销
                ...
                String ysStr = "欠款:";
                BigDecimal ys = new BigDecimal(ysAmount);
                if (ys.compareTo(BigDecimal.ZERO) == -1) {
                    ysStr = "欠款转预收:";
                }
                pUtil.printLargeText(ysStr + ysAmount, 0);
                pUtil.printLine();
                pUtil.printLargeText("实付金额:" + sfAmount, 0);
                pUtil.printLine();
            }
    ...
    public static <T> boolean printPlaceAnOrderList(BluetoothSocket bluetoothSocket, ClientBean clientBean, String title, String orderCode,
                                                    String salesmanName, String salesmanPhone, T t, T t1) {
        ...
        try {
            ...
            PlaceAnOrderDetailBean orderBean = (PlaceAnOrderDetailBean) t1;
    ...
    private static void bottomPrint(PrintUtil pUtil, PrintParamBean paramBean) throws IOException {
        ...
        pUtil.printTwoColumnWeight("日期:", "客户签字:");
        if (paramBean.getRemarks() != null && paramBean.getRemarks().length() > 0) {
