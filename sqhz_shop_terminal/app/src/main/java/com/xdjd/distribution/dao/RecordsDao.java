...
public class RecordsDao {
     ...
     * @param data
     */
    public void update(RecordsBean data) {
        SQLiteDatabase sqlite = mDatabaseHelper.getWritableDatabase();
        String sql = ("update " + DatabaseHelper.ACCOUNT_RECORD_NAME +
                " set password=?, login_time=? where account=?");
        sqlite.execSQL(sql, new String[]{data.getPassword(),
                data.getLogin_time(),data.getAccount()});
        sqlite.close();
    }
    ...
    public List<RecordsBean> getRecordsList() {
        ...
        Cursor cursor = sqlite.rawQuery("SELECT * FROM "
                + DatabaseHelper.ACCOUNT_RECORD_NAME + " ORDER BY login_time DESC" , null);
