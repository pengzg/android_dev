...
public abstract class LoadingLayout extends FrameLayout implements
	 ...
	 * 左侧图标布局
	 */
	private RelativeLayout rlLeftImg;
	/**
	 * 右侧文字提示布局
	 */
	private LinearLayout llRightDesc;
	...
	public LoadingLayout(Context context, final Mode mode,
						 final Orientation scrollDirection, boolean isShowRight,TypedArray attrs) {
		super(context);
		this.context = context;
		mMode = mode;
		mScrollDirection = scrollDirection;
		isShowRightLayout = isShowRight;
		...
		rlLeftImg = (RelativeLayout) mInnerLayout.findViewById(R.id.rl_left_img);
		llRightDesc = (LinearLayout) mInnerLayout.findViewById(R.id.ll_right_desc);
		...
		if (isShowRightLayout){
			llRightDesc.setVisibility(View.VISIBLE);
			lp2.width = LayoutParams.WRAP_CONTENT;
			lp2.leftMargin = dp2px(47);
			lp2.gravity = Gravity.CENTER_VERTICAL|Gravity.LEFT;  //这才是布局文件中的Android:layout_gravity属性
			rlLeftImg.setLayoutParams(lp2);
		}else{
			llRightDesc.setVisibility(View.GONE);
			//该布局在LinearLayout下
			lp2.gravity = Gravity.CENTER;  //这才是布局文件中的Android:layout_gravity属性
			lp2.leftMargin = 0;
			lp2.width = LayoutParams.MATCH_PARENT;
			rlLeftImg.setGravity(Gravity.CENTER);
			rlLeftImg.setLayoutParams(lp2);
//			textview.setGravity(Gravity.CENTER); //这是布局文件中的Android：gravity属性
	...
	public final void pullToRefresh() {
		...
		mSubHeaderText.setText("最后更新时间" + getDate());
	...
	public static String getDate() {
		...
		sdf = new SimpleDateFormat("yyyy年MM月dd日 ah:mm:ss");
		return sdf.format(d);
	}
	 ...
	 * @param isRightLayout
	 */
	public void updateLayout(boolean isRightLayout){
		FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mInnerLayout
				.getLayoutParams();
		FrameLayout.LayoutParams lp2 = new FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
		if (isRightLayout){
