...
public class SalesmanHomePagerFragment extends BaseFragment implements CalendarSelectPopup.ItemDateOnListener {
    ...
    @BindView(R.id.ll_ysjb_main)
    LinearLayout mLlYsjbMain;
    @BindView(R.id.ll_ysqs_main)
    LinearLayout mLlYsqsMain;
    //营收简报起止时间
    ...
    public void OnActCreate(Bundle savedInstanceState) {
        ...
        if (Common.ROLE3002.equals(userBean.getRoleCode())){//客户主管
            mLlYsjbMain.setVisibility(View.GONE);
            mLlYsqsMain.setVisibility(View.GONE);

            loadYhqs();
        }else{
            mLlYsjbMain.setVisibility(View.VISIBLE);
            mLlYsqsMain.setVisibility(View.VISIBLE);
        ...
        mPullScroll.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener<ScrollView>() {
            ...
            public void onRefresh(PullToRefreshBase<ScrollView> refreshView) {
                if (!Common.ROLE3002.equals(userBean.getRoleCode())){//非客户主管
                    loadYsjb();
                    loadYsqs();
                    loadYhqs();
                }else{
    ...
    @OnClick({R.id.tv_ysjb_select, R.id.tv_ysqs_select, R.id.btn_order_amount, R.id.btn_order_num, R.id.ll_ysjb,
            R.id.ll_ysqs, R.id.ll_store, R.id.ll_order_search, R.id.ll_apply_member,
            R.id.tv_yhqs_select, R.id.ll_yhqs, R.id.ll_order_group, R.id.ll_ask})
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_ysqs_select://营收趋势
                ...
                mTimePickerUtil.showTimePicker(main.getMainId(), mTvYsqsDate.getText().toString().split("-")[0],
                        mTvYsqsDate.getText().toString().split("-")[1], false,false);
                break;
            case R.id.ll_ysqs:
                datePrickType = 2;
                mTimePickerUtil.showTimePicker(main.getMainId(), mTvYsqsDate.getText().toString().split("-")[0],
            ...
            case R.id.tv_yhqs_select:
                ...
                mTimePickerUtil.showTimePicker(main.getMainId(), mTvYhqsDate.getText().toString().split("-")[0],
                        mTvYhqsDate.getText().toString().split("-")[1], false,false);
                break;
            case R.id.ll_yhqs:
                datePrickType = 3;
                mTimePickerUtil.showTimePicker(main.getMainId(), mTvYhqsDate.getText().toString().split("-")[0],
        ...
        AsyncHttpUtil<BriefingBean> httpUtil = new AsyncHttpUtil<>(getActivity(), BriefingBean.class, new IUpdateUI<BriefingBean>() {
            ...
            public void updata(BriefingBean jsonBean) {
                if ("200".equals(jsonBean.getCode())) {
                    ...
                    mTvNum.setText(bean.getSendNum() + "");
                    // 设置数据
                    mTvNum.withNumber(bean.getSendNum());
                    // 设置动画播放时间
    ...
    private void setYhqsData(List<TrendsChartBean> dataList) {
        ...
        XAxis xAxis = mChartYhLine.getXAxis();
        xAxis.setValueFormatter(new IndexAxisValueFormatter(getMonths(dataList, dateYhqs)));
